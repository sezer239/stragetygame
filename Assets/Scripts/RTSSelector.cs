...
public class RTSSelector : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(mouseToWorldRay, out hitInfo))
            {
                ...
                if (s == null)
                    s = hitInfo.collider.gameObject.GetComponent<Selectable>();
    ...
    public List<NPCLogic> GetSelectedAsNPCLogic()
    {
        List<NPCLogic> result = new List<NPCLogic>();
        foreach (var selectable in selectables)
        {
            if (selectable.isSelected)
            {
                var npcLogic = selectable.GetComponent<NPCLogic>();
                if (npcLogic != null) result.Add(npcLogic);
            }
        }
        return result;
    }
